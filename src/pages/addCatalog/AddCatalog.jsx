...
// import { useCreateCatalogMutation, useCreateExcelCatalogMutation, useSearchCatalogQuery } from "../../context/api/catalogApi";
...
//     const [CreateExcelCatalog, { data }] = useCreateExcelCatalogMutation()
//     const { formData, setFormData, handleChange } = useGetValue(initialState);
//     const { data: searchResults } = useSearchCatalogQuery();
...
//                     <button className="btn-add" onClick={() => setHide(true)}>
//                         + Import Excel
//                     </button>
//                 </div>
//             </div>
...
//             {hide && (
//                 <Module close={setHide} width={490} bg={"#aaa9"}>
//                     <div className="excel">
//                         <div className="drop-zone" onClick={() => document.getElementById("excelInput").click()}>
//                             <input type="file" id="excelInput" accept=".xlsx,.xls,.csv" />
//                             <span className="drop-zone__icon"><MdOutlineCloudUpload /></span>
//                             <p className="drop-zone__title">Drop your file here</p>
//                             <p className="drop-zone__hint">or <span>browse from computer</span></p>
//                         </div>
//                     </div>
//                 </Module>
//             )}
...
import FilterCatalog from "../../components/filterCatalog/FilterCatalog";
import Module from "../../components/module/Module";
import { MdOutlineDriveFolderUpload, MdOutlineCloudUpload } from "react-icons/md";
import { useCreateCatalogMutation, useCreateExcelCatalogMutation } from "../../context/api/catalogApi";
import { useGetValue } from "../../hooks/useGetValue";
...
const AddCatalog = () => {
    ...
    const [CreateCatalog, { isLoading }] = useCreateCatalogMutation();
    const [createExcelCatalog, { isLoading: isImporting }] = useCreateExcelCatalogMutation();
    const { formData, setFormData, handleChange } = useGetValue(initialState);
    ...
    const uploadExcelFile = async (file) => {
        ...
        if (!isExcelFile(file)) {
            setImportError("Faqat .xlsx yoki .xls fayl yuklang");
            return;
        }

        setImportError("");
        setImportStatus(`Yuklanmoqda: ${file.name}...`);

        const data = new FormData();
        data.append("file", file);

        try {
            const result = await createExcelCatalog(data).unwrap();
    ...
    const handleSubmit = (e) => {
        ...
        CreateCatalog(data)
            .unwrap()
            .then(() => { setIsAddOpen(false); resetForm(); })
            .catch((err) => console.error("Error:", err));
    };
    ...
    return (
        <div className="catalog-page">
            <div className="catalog-topbar">
                ...
                <div className="catalog-topbar-btns">
                    <button type="button" className="btn-add" onClick={() => setIsImportOpen(true)}>
                        + Import Excel
                    </button>

                    <button type="button" className="btn-add" onClick={() => setIsAddOpen(true)}>
                        + Add Product
                    </button>
                </div>
            ...
            {isImportOpen && (
                <Module close={closeImportModal} width={480} bg={"#aaa9"}>
                    <div className="import-modal">
                        <div
                            className={`import-dropzone${importDragging ? " import-dropzone--active" : ""}${isImporting ? " import-dropzone--loading" : ""}`}
                            onDragOver={(e) => {
                                e.preventDefault();
                            ...
                            onClick={() => !isImporting && excelInputRef.current?.click()}
                            role="button"
                            tabIndex={0}
                            onKeyDown={(e) => {
                                if (e.key === "Enter" || e.key === " ") {
                        ...
                        {importStatus && <p className="import-modal__status">{importStatus}</p>}
                        {importError && <p className="import-modal__error">{importError}</p>}
                    </div>
                </Module>
            )}

            {isAddOpen && (
                <Module close={() => { setIsAddOpen(false); resetForm(); }} width={550} bg={"#aaa9"}>
                    <form onSubmit={handleSubmit} className="modal">
                        ...
                        <div className="modal__footer">
                            <button type="button" className="btn-cancel" onClick={() => { setIsAddOpen(false); resetForm(); }}>Cancel</button>
                            <button type="submit" className="btn-save" disabled={isLoading}>
                                {isLoading ? "Saving..." : "✓ Save"}
                            </button>
                        </div>
