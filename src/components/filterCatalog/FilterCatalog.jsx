...
function EditModal({ item, onClose, onSave, onUpdate }) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            setError(err?.data?.message || "Saqlashda xatolik yuz berdi.");
        } finally {
            setIsSaving(false);
        }
    };
        ...
        <div className="modal-backdrop" onClick={onClose} role="dialog" aria-modal aria-label="Edit item">
            <div className="modal-box modal-box--edit" onClick={e => e.stopPropagation()}>
                ...
                <div className="modal-body">
                    <div className="edit-grid">
                        ...
                        <div className="edit-field edit-field--full">
                            ...
                            <input type="file" accept="image/*" className="edit-input" onChange={handlePhotoChange} disabled={isSaving} />
                            {photoPreview && (
                                <img src={photoPreview} style={{ width: 120, height: "auto", marginTop: 8, borderRadius: 4 }} alt="Preview" className="edit-photo-preview" />
                            )}
                        </div>
                    </div>
                ...
                <div className="modal-footer">
                    <button className="modal-btn modal-btn--cancel" onClick={onClose} disabled={isSaving}>Cancel</button>
                    <button className="modal-btn modal-btn--save" onClick={handleSave} disabled={isSaving}>
    ...
    return (
        <div className="modal-backdrop" onClick={onClose} role="dialog" aria-modal aria-label="Delete item">
            <div className="modal-box modal-box--delete" onClick={e => e.stopPropagation()}>
                <div className="modal-head">
                    <div className="modal-head-icon modal-head-icon--delete"><DeleteIcon /></div>
                    <div>
...
export default function FilterCatalog({ hide }) {
    ...
    // hide=true bo'lsa actions ustunini olib tashlaymiz
    const columns = useMemo(
        () => hide ? ALL_COLUMNS.filter(c => c.key !== "actions") : ALL_COLUMNS,
        [hide]
    );

    const filteredData = useMemo(() => filterCatalogItems(localData, search), [localData, search]);

    useEffect(() => { setPage(1); }, [search]);

    const handlePreview = useCallback(({ src, trt }) => setModalPhoto({ src, trt }), []);
    const handleSave = useCallback((updated) => setLocalData(prev => prev.map(p => p.id === updated.id ? updated : p)), []);
    const handleConfirmDelete = useCallback((item) => setLocalData(prev => prev.filter(p => p.id !== item.id)), []);

    const handleExportExcel = useCallback(async () => {
        if (!filteredData.length) return;
        setExporting(true);
        try { await exportCatalogExcel(filteredData); } finally { setExporting(false); }
    }, [filteredData]);

    const handleExportPdf = useCallback(async () => {
        if (!filteredData.length) return;
        setExportingPDF(true);
        try { await exportCatalogPdf(filteredData); } finally { setExportingPDF(false); }
    }, [filteredData]);
    ...
    return (
        <div className="fc">
            ...
            <div className="fc-filters">
                ...
                <div className="fc-counter" aria-live="polite"><b>{filteredData.length}</b></div>
            </div>
                    ...
                    <thead>
                        <tr>
                            {columns.map(({ key, label }) => (
                                <th key={key} scope="col"><span className="th-inner">{label}</span></th>
                            ))}
                        </tr>
                    </thead>
                    <tbody>
                        {isLoading ? (
                            <tr><td colSpan={columns.length} className="fc-empty"><div className="fc-empty-inner"><p>Loading...</p></div></td></tr>
                        ) : isError ? (
                            <tr><td colSpan={columns.length} className="fc-empty"><div className="fc-empty-inner"><p>Failed to load data</p></div></td></tr>
                        ) : pageData.length === 0 ? (
                            <tr>
                            ...
                            <tr key={`${p.id ?? "row"}-${i}`} style={{ "--row-i": i }}>
                                ...
                                <td className="cell-oem">
                                    {(Array.isArray(p.oemNo) ? p.oemNo : [p.oemNo]).map(o => <div key={o}>{o}</div>)}
                                </td>
                                <td className="cell-ctr"><span className="ctr-tag">{p.ctrNo}</span></td>
                                ...
                                <td className="cell-sm">{(Array.isArray(p.carName) ? p.carName : [p.carName]).join(", ")}</td>
                                <td className="cell-sm cell-muted">{(Array.isArray(p.model) ? p.model : [p.model]).join(", ")}</td>
                                <td className="cell-sm">{(Array.isArray(p.years) ? p.years : [p.years]).join(", ")}</td>
                                <td><PhotoCell trt={p.trtNo} photo={p.photo} onPreview={handlePreview} /></td>
                                <td className="cell-weight">
                                    {typeof p.weightPerPcKg === "number" ? p.weightPerPcKg.toFixed(3) : p.weightPerPcKg}
                                </td>
                                {!hide && (
                                    <td className="cell-actions">
                                        <RowMenu item={p} onEdit={setEditItem} onDelete={setDeleteItem} />
                                    </td>
                                )}
                            </tr>
                        ))}
            ...
            {modalPhoto && <PhotoModal src={modalPhoto.src} trt={modalPhoto.trt} onClose={() => setModalPhoto(null)} />}
            {editItem && <EditModal item={editItem} onClose={() => setEditItem(null)} onSave={handleSave} onUpdate={updateCatalog} />}
            {deleteItem && <DeleteModal item={deleteItem} onClose={() => setDeleteItem(null)} onConfirm={handleConfirmDelete} />}
        </div>
    );
}
