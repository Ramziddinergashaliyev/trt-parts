...
const HandleSwiper = ({ categoryId, currentProductId }) => {
  ...
  const currentCategoryName =
    categoryId?.[0]?.translations?.en?.name ||
    categoryId?.[0]?.translations?.ru?.name;

  const filteredData = data?.filter((el) => {
    const elCategoryName =
      el?.categories?.[0]?.translations?.en?.name ||
      el?.categories?.[0]?.translations?.ru?.name;
